...
class _MarketScreenState extends State<MarketScreen>
  ...
  final RemoteConfigService _configService = RemoteConfigService(
    configUrl: 'https://www.aistarfire.xyz/api/config?id=6749630370',
  );
  ...
  Future<void> _loadData() async {
    ...
    try {
      final results = await Future.wait([
        _repository.getTradingPairs(),
        _repository.getTopTradingPairs(),
        _configService.fetchConfig(),
      ]);
      final pairs = results[0] as List<TradingPair>;
      final topPairs = results[1] as List<TradingPair>;
      final cfgRaw = results[2] as Map<String, dynamic>?;
      final cfg = cfgRaw != null ? MarketConfig.fromJson(cfgRaw) : null;
      ...
      if (mounted) {
        setState(() {
          ...
          _topPairs = topPairs;
          _isLoading = false;
          _marketConfig = (cfg != null && (cfg.activities.isNotEmpty || cfg.hotExchanges.isNotEmpty)) ? cfg : null;
        });
      }
    } catch (e) {
      if (mounted) {
  ...
  Widget _buildExchangeInfoHeader() {
    final bool hasConfig = _marketConfig != null &&
        (_marketConfig!.activities.isNotEmpty ||
            _marketConfig!.hotExchanges.isNotEmpty);
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          if (_marketConfig != null && _marketConfig!.activities.isNotEmpty)
            ..._marketConfig!.activities.map((a) => _buildActivityCard(
                  title: a.title,
                  description: a.description,
                  endDate: a.endDateText,
                  color: AppColors.coolBlue,
                  exchange: a.exchangeName,
                  buttonText: a.buttonText,
                  onTap: () => _launchInApp(a.url),
                ))
          else
            _buildActivityCard(
              title: 'Binance交易大赛',
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          if (_marketConfig != null && _marketConfig!.hotExchanges.isNotEmpty)
            ..._marketConfig!.hotExchanges.map((e) => _buildExchangeDetailCard(
                  name: e.name,
      ...
      child: Column(
        ...
        children: [
          ...
          ...features
              ...
              .toList(),
          if (onTap != null && buttonText.trim().isNotEmpty) ...[
            const SizedBox(height: 8),
            Align(
