...
class NewsRepository {
  ...
  /// 获取新闻事件列表，使用Cointelegraph GraphQL接口（latest-news, cn）
  Future<List<NewsEvent>> getNewsEvents() async {
    ...
    try {
      final fetched = await _fetchCointelegraphLatestNews(length: 30, offset: 0);
      allEvents = fetched;
      // 更新缓存
  ...
  Future<List<Map<String, dynamic>>> getMarketTrends() async {
    try {
      // 此处暂无外部真实接口，保留原逻辑
      final trends = _getMockMarketTrends();
  ...
  /// 获取货币对详细分析（移除外部AI接口，保留本地模拟和缓存逻辑）
  Future<Map<String, dynamic>> getCurrencyPairAnalysis(String pair) async {
    if (_analysisCache.containsKey(pair) && _analysisCacheTime.containsKey(pair)) {
    ...
    final analysis = _getMockAnalysis(pair);
    _analysisCache[pair] = analysis;
    _analysisCacheTime[pair] = DateTime.now();
    return analysis;
  }

  Future<List<NewsEvent>> _fetchCointelegraphLatestNews({int length = 30, int offset = 0}) async {
    final uri = Uri.parse('https://conpletus.cointelegraph.com/v1/');
    final payload = {
      'operationName': 'CategoryPagePostsQuery',
      'query':
          'query CategoryPagePostsQuery(\$short: String, \$slug: String!, \$offset: Int = 0, \$length: Int = 10, \$hideFromMainPage: Boolean = null) {\n locale(short: \$short) {\n category(slug: \$slug) {\n id\n posts(\n order: \n"postPublishedTime"\n offset: \$offset\n length: \$length\n hideFromMainPage: \$hideFromMainPage\n ) {\n data {\n id\n slug\n views\n postTranslate {\n id\n title\n avatar\n published\n publishedHumanFormat\n leadText\n author {\n id\n slug\n innovationCircleUrl\n authorTranslates {\n id\n name\n __typename\n }\n __typename\n }\n __typename\n }\n category {\n id\n slug\n categoryTranslates {\n id\n title\n __typename\n }\n __typename\n }\n author {\n id\n slug\n authorTranslates {\n id\n name\n __typename\n }\n __typename\n }\n postBadge {\n id\n label\n postBadgeTranslates {\n id\n title\n __typename\n }\n __typename\n }\n showShares\n showStats\n __typename\n }\n postsCount\n __typename\n }\n __typename\n }\n __typename\n }\n}',
      'variables': {
    ...
    if (resp.statusCode != 200) {
      throw Exception('GraphQL status ${resp.statusCode}');
    }
    final decoded = jsonDecode(resp.body) as Map<String, dynamic>;
    final List<dynamic> posts =
        (((decoded['data'] ?? {}) as Map)['locale'] ?? {})['category']?['posts']?['data']
                as List<dynamic>? ??
            [];
    ...
    return posts.map((p) {
      ...
      final t = (m['postTranslate'] ?? {}) as Map<String, dynamic>;
      final title = (t['title'] ?? '').toString();
      final summary = (t['leadText'] ?? '').toString();
      final published = (t['published'] ?? '').toString();
      final author = ((t['author'] ?? {}) as Map<String, dynamic>);
      final authorName = ((author['authorTranslates'] as List?)?.isNotEmpty ?? false)
          ? (((author['authorTranslates'] as List).first as Map)['name'] ?? '').toString()
          : '';

      final impactPool = ['High', 'Medium', 'Low'];
      final impact = impactPool[random.nextInt(impactPool.length)];
      final affected = _guessAffectedPairs(title, summary);
      final sentiment = (random.nextDouble() * 100).clamp(30, 90).toDouble();

      return NewsEvent(
        title: title,
        timeAgo: published.isNotEmpty ? published : '最新',
        summary: summary.isNotEmpty ? summary : title,
        impact: impact,
        affectedPairs: affected,
        source: authorName.isNotEmpty ? authorName : 'Cointelegraph',
        url: '',
        imageUrl: '',
        sentiment: sentiment,
      );
  ...
  List<String> _guessAffectedPairs(String title, String summary) {
    final text = '$title $summary'.toUpperCase();
    ...
    for (final c in coins) {
      if (text.contains(c)) res.add('$c/USD');
    }
    if (res.isEmpty) res.add('BTC/USD');
    return res.toList();
  }
